...
void Chatting::createConnection() {
    ...
    std::cout << "Trying to connect to chat server at " << serverAddress << ":" << port << "...\n";
    ...
    try {
        ...
        auto line = std::shared_ptr<char[]>(new char[chat::Message::MAX_BODY_LENGTH + 1]);

        while(std::cin.getline(line.get(), chat::Message::MAX_BODY_LENGTH + 1)) {
            auto msg = chat::Message();
            msg.bodyLength(std::strlen(line.get()));

            std::memcpy(msg.body(), line.get(), msg.bodyLength());
            msg.encodeHeader();
...
void Chatting::openChatServer() {
    ...
    _serverThread = std::make_unique<std::thread>([this, port]() {
        try {
            auto ioContext  = IOContext();
            auto tcpV4      = boost::asio::ip::tcp::v4();
            
            auto endpoint = TcpEndpoint(tcpV4, port);
